...
{
int i=0,f=0,d1=0,d2=0,n=0,nmax=0,***GRID; /* amino acid position iterator */
...
//printf("\t\tadding data to bin d1=%d d2=%d \n",d1,d2);
			GRID[i][d1][d2]++;
			if(GRID[i][d1][d2]>nmax) nmax=GRID[i][d1][d2];
			}
		}
...
for (i=0;i<4;i++) 
	...
	for (d2=0;d2<NG;d2++)
		...
		for (d1=0;d1<NG;d1++)
			{
			fprintf(BINS[i].F,"%10.5f ",(double)GRID[i][d1][d2]/(double)nmax);
			}
		fprintf(BINS[i].F,"\n");
		}
	}
